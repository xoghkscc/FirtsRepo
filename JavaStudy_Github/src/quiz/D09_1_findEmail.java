...
public class D09_1_findEmail {
	//이메일을 판별할 수 있는 정규표현식을 작성하고 테스트해보세요
	
	public static void main(String[] args) {
		String email="xoghkscc@naver.com";
		System.out.println(Pattern.matches("[a-zA-Z0-9]{5,}@[a-zA-Z]+(.com|.net|.co.kr)", email));
		
		
		//다음 문자열에서 이메일만 골라서 ArrayList에 차곡차곡 정리한 후 한번씩 출력
		//제대로된 이메일이 몇개인지도 세어보기
		
		String email_text = "조선일보	가정/육아	박은주	zeeny@chosun.com\r\n" + 
				...
				"매일경제	　	황시영	siyooung@shinyandcloose";
		
		Pattern regex = Pattern.compile("[\\w]+@[\\w]+[.](com|net|co.kr)");
		Matcher matcher = regex.matcher(email_text);
		ArrayList<String> email2 = new ArrayList<String>();
		int cnt=0;
		while(matcher.find()) {
			email2.add(matcher.group());
			cnt++;
		}
